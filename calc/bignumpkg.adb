...
package body BigNumPkg is
    ...
    function  validLength (Item: BigNum) return Integer is
    ...
    function toString(X: BigNum) return String is
        BigNumString: String(1..validLength(x));
    begin
        -- Write out the digits of the number.
        for I in validLength(x)..Size-1 loop
            BigNumString(I):= Character'Val(X(I));
        end loop;
